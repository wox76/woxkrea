...
async function initAI() {
    try {
        ...
        // Carichiamo il modello Image-to-Image (es. LCM o SD Turbo)
        // 'webgpu' è fondamentale per la velocità
        model = await pipeline('image-to-image', 'Xenova/stable-diffusion-v1-5', {
            device: 'webgpu', 
        });
        
        status.textContent = "IA Pronta!";
    } catch (err) {
        status.textContent = "Errore: WebGPU non supportata?";
        console.error(err);
    }
}
